		...
		function () {
			...
			if (bad.length) {
					...
					showMessage(
						$input,
						'error',
						'Invalid file type selected. Allowed: ' + allowed.split( ',' ).map(
							function (x) {
								return x.trim();}
						).filter( Boolean ).join( ', ' ) + '.'
					);
